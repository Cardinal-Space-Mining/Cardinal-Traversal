...
namespace GridUtils
{
	...
	inline static Eigen::Vector2<IntT> worldToGrid(FloatT x, FloatT y, const Eigen::Vector2<FloatT> &off, FloatT res)
	{
	...
	template <typename IntT = int, typename FloatT = float>
	inline static Eigen::Vector2<IntT> worldToGrid(const Eigen::Vector4<FloatT> &pt, const Eigen::Vector2<FloatT> &off, FloatT res)
	{
		return worldToGrid<IntT, FloatT>(pt.x(), pt.y(), off, res);
	}
...
public:
            ...
            WeightT weight_threshold)
    {
        ...
        const This_T::Vec2m
            ...
            pos = GridUtils::worldToGrid<mapsize_t, double>(pose_x, pose_y, origin, cell_resolution);
...
//        using Vec2m = Eigen::Vector2<mapsize_t>; // represents a location on the map
		...
		WeightT turn_cost = static_cast<WeightT>(1))
	{
		...
		const This_T::Vec2m
			wsize{wsize_x, wsize_y},
			start = GridUtils::worldToGrid<mapsize_t, double>(start_x, start_y, origin, cell_resolution),
			end = GridUtils::worldToGrid<mapsize_t, double>(end_x, end_y, origin, cell_resolution);
